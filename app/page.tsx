...
export default async function HomePage() {
  ...
  return (
    <>
      ...
      <section className="relative min-h-screen flex items-stretch overflow-hidden bg-white pt-24 lg:pt-28">
        <div className="w-full grid grid-cols-1 md:grid-cols-2">
          ...
          <div className="flex flex-col justify-center px-8 sm:px-12 lg:px-16 py-16 bg-white">
            ...
            <div className="flex items-center gap-2 mb-5">
              ...
              <span className="font-body text-xs uppercase tracking-widest text-[var(--color-text-mid)] font-medium">
            ...
            <h1 className="leading-none mb-5">
              <span className="block font-display italic text-[var(--color-amber)] text-3xl">The</span>
              <span className="block font-display font-bold text-[var(--color-text-dark)] text-7xl md:text-8xl">Coffee</span>
              <span className="block font-display font-bold text-[var(--color-orange)] text-7xl md:text-8xl">Marathon</span>
            </h1>

            {/* Theme */}
            <div className="flex flex-col gap-1 bg-[var(--color-amber)]/10 border border-[var(--color-amber)]/30 rounded-xl px-4 py-3 mb-5 max-w-sm">
              <span className="font-body text-xs font-bold text-[var(--color-amber)] uppercase tracking-widest">Theme 2026</span>
              <p className="font-body text-sm font-semibold text-[var(--color-text-dark)] leading-snug italic">
            ...
            {/* Location */}
            <div className="flex items-center gap-2 mb-5">
              <MapPin size={14} className="text-[var(--color-amber)] shrink-0" />
            ...
            <div className="flex items-center gap-3 mb-2">
              <span className="font-body text-[var(--color-text-light)] text-xs tracking-widest uppercase">Featuring</span>
              <span className="w-12 h-px bg-[var(--color-border)]" />
            </div>
            <div className="flex items-center gap-3 mb-7">
              <div className="inline-flex items-center gap-1.5 bg-[#F5C842] rounded-full px-2.5 py-0.5">
                <span className="font-body text-xs font-bold text-[#1A0A00] uppercase tracking-wide">Live in Concert</span>
              </div>
              <p className="font-display font-bold text-[var(--color-text-dark)] text-3xl leading-tight">Tyrese Gibson</p>
            </div>
            ...
            <div className="flex flex-wrap gap-4">
              <a
                ...
                className="inline-flex items-center bg-[var(--color-amber)] text-white px-6 py-3 rounded-full font-body font-semibold text-sm hover:bg-[var(--color-amber-dark)] transition-colors shadow-sm"
              >
                Register Now →
              </a>
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 px-6 py-3 rounded-full border-2 border-[var(--color-border)] text-[var(--color-text-dark)] font-body font-semibold text-sm hover:border-[var(--color-amber)] hover:text-[var(--color-amber)] transition-all"
              >
                Get Your Kit
              </a>
            </div>
          ...
          <div className="relative hidden md:block min-h-[600px]">
            ...
            <div className="absolute inset-0 bg-gradient-to-t from-[#1A0A00]/80 via-[#1A0A00]/10 to-transparent" />
            {/* Left fade to white */}
            <div className="absolute inset-y-0 left-0 w-20 bg-gradient-to-r from-white to-transparent" />

            {/* Headline badge — top right */}
            <div className="absolute top-8 right-8">
              <div className="inline-flex items-center gap-1.5 bg-[#F5C842] rounded-full px-3 py-1.5 shadow-lg">
                <span className="font-body text-xs font-bold text-[#1A0A00] uppercase tracking-wide">Headline Performer</span>
              </div>
            </div>

            {/* Date badge — top left (just inside the image) */}
            <div className="absolute top-8 left-24 bg-white/90 backdrop-blur-sm rounded-2xl px-5 py-3 text-right shadow-lg">
              <p className="font-body text-[var(--color-text-light)] text-xs tracking-widest uppercase">Race Day</p>
              <p className="font-display font-bold text-[var(--color-amber)] text-xl leading-none">3 OCT</p>
              <p className="font-display font-bold text-[var(--color-text-dark)] text-4xl leading-none">2026</p>
            </div>

            {/* Artist name overlay — bottom */}
            <div className="absolute bottom-8 left-8 right-8">
              <p className="font-body text-white/60 text-xs uppercase tracking-widest mb-1">Featuring</p>
        ...
        <div className="absolute top-[100px] right-4 bg-white/90 backdrop-blur-sm rounded-xl px-4 py-3 text-right shadow-lg md:hidden">
          <p className="font-display font-bold text-[var(--color-amber)] text-lg leading-none">3 OCT</p>
          <p className="font-display font-bold text-[var(--color-text-dark)] text-3xl leading-none">2026</p>
        </div>
      </section>

      {/* ── 2. STATS BAR ────────────────────────────────────────────── */}
      <section className="bg-[var(--color-amber)] py-8">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="grid grid-cols-2 lg:grid-cols-4 divide-x divide-white/30">
            {eventStats.map((stat: { number: string; label: string }) => (
              <div key={stat.label} className="text-center py-2 px-4">
                <p className="font-display text-3xl font-bold text-white">{stat.number}</p>
                <p className="font-body text-xs text-white/80 uppercase tracking-widest mt-1">{stat.label}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            <div className="flex items-center justify-center gap-3 mb-4">
              ...
              <span className="font-body text-xs uppercase tracking-widest text-[var(--color-amber)] font-semibold">Race Categories</span>
              <span className="w-8 h-0.5 bg-[var(--color-amber)]" />
            </div>
            <h2 className="font-display text-4xl md:text-5xl font-bold text-[var(--color-text-dark)]">
          ...
          <div className="text-center mt-10">
            <Link href="/races" className="inline-flex items-center gap-2 font-body text-sm font-semibold text-[var(--color-amber)] hover:text-[var(--color-amber-dark)] transition-colors group">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <div className="flex flex-wrap justify-center items-center gap-6 mb-6">
            {[
              ...
              <div key={label} className="flex items-center gap-2 px-5 py-3 bg-white rounded-xl border border-[var(--color-border)] shadow-sm">
                {logo && <Image src="/images/logo.png" alt="The Coffee Marathon" width={32} height={32} className="object-contain" />}
                <span className="font-body text-sm font-semibold text-[var(--color-text-dark)]">{label}</span>
              </div>
            ))}
            {(sponsors ?? []).map((s: Sponsor) => (
              <div key={s._id} className="px-5 py-3 bg-white rounded-xl border border-[var(--color-border)] shadow-sm">
                <span className="font-body text-sm font-semibold text-[var(--color-text-dark)]">{s.name}</span>
              </div>
            ))}
          </div>
          <Link href="/sponsorships" className="inline-flex items-center gap-2 font-body text-sm font-semibold text-[var(--color-amber)] hover:text-[var(--color-amber-dark)] transition-colors">
      ...
      <section className="relative py-24 overflow-hidden">
        <div className="absolute inset-0">
          ...
          <div className="absolute inset-0 bg-[var(--color-amber)]/80" />
        </div>
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 text-center">
          ...
          <div className="flex flex-wrap items-center justify-center gap-4">
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center bg-white text-[var(--color-text-dark)] px-8 py-4 rounded-full font-body font-bold text-base hover:bg-[var(--color-amber-light)] transition-colors shadow-lg"
            >
              Register Now →
            </a>
            <Link
              href="/races"
              className="inline-flex items-center gap-2 px-8 py-4 rounded-full border-2 border-white text-white font-body font-semibold text-base hover:bg-white/10 transition-all"
            >
              View all races
            </Link>
          </div>
